...
TTSProvider = Literal["elevenlabs", "deepgram", "sarvam", "cartesia", "dograh", "rime", "qwentts", "pocket-tts"]
...
async def get_voices(
    ...
    if provider == "pocket-tts":
        # System defaults
        voices = [
            VoiceInfo(voice_id="alba", name="Alba (Default)", description="Balanced female voice", gender="female", accent="British"),
            VoiceInfo(voice_id="emma", name="Emma (Default)", description="Warm female voice", gender="female", accent="American"),
            VoiceInfo(voice_id="expresso", name="Expresso (Default)", description="Fast expressive voice", gender="female"),
            VoiceInfo(voice_id="amanda", name="Amanda (Default)", description="Clear professional voice", gender="female"),
        ]
        
        # Scan shared volume for cloned voices
        voices_dir = "/voices"
        if os.path.exists(voices_dir):
            for f in os.listdir(voices_dir):
                if f.endswith(".safetensors") or f.endswith(".wav"):
                    voices.append(VoiceInfo(
                        voice_id=os.path.join("/voices", f),
                        name=f"Cloned: {f}",
                        description=f"Local cloned voice from {f}",
                    ))
        
        return VoicesResponse(provider="pocket-tts", voices=voices)
        ...
        return VoicesResponse(
            ...
            voices=[
                VoiceInfo(
                    ...
                    description="Cloned voice from your custom endpoint",
                )
            ],
        )
