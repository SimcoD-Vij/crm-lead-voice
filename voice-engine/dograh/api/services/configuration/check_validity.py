...
class UserConfigurationValidator:
    def __init__(self):
        self._validator_map = {
            ...
            ServiceProviders.QWENTTS.value: self._check_qwentts_api_key,
            ServiceProviders.OLLAMA.value: self._check_ollama_api_key,
            ServiceProviders.POCKETTTS.value: self._check_pocket_tts_api_key,
        }
    ...
    def _validate_service(
        ...
        try:
            if not self._check_api_key(provider, api_key):
                # Ensure provider is stringified correctly (handles Enum members)
                provider_str = str(provider.value) if hasattr(provider, "value") else str(provider)
                return [
                    {"model": service_name, "message": f"Invalid {provider_str} API key"}
                ]
        except ValueError as e:
            return [{"model": service_name, "message": str(e)}]
    ...
    def _check_dograh_api_key(self, model: str, api_key: Optional[str]) -> bool:
        if not api_key:
            return True
    ...
    def _check_qwentts_api_key(self, model: str, api_key: Optional[str]) -> bool:
        # For local custom endpoints, an API key is often not required. 
        # If one is provided, we assume it's valid for now.
        return True

    def _check_ollama_api_key(self, model: str, api_key: Optional[str]) -> bool:
        return True

    def _check_pocket_tts_api_key(self, model: str, api_key: Optional[str]) -> bool:
        return True
