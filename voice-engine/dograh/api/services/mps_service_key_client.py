...
class MPSServiceKeyClient:
    ...
    def validate_service_key(
        ...
        In OSS/local mode, skips the remote MPS call entirely.
        """
        # In OSS mode, MPS (services.dograh.com) is unreachable locally.
        # Skip validation — trust the configured key and allow the call.
        if DEPLOYMENT_MODE == "oss":
            logger.info("OSS mode: skipping Dograh service key remote validation")
            return True
        try:
