...
async def check_dograh_quota(user: UserModel) -> QuotaCheckResult:
    ...
    try:
        ...
        for api_key in dograh_api_keys:
            if not api_key:
                logger.info("Skipping Dograh credit check for missing API key (local/OSS mode)")
                continue

            # In OSS/local mode, the MPS endpoint (services.dograh.com) is unreachable.
            # Skip the remote credit check to allow calls to proceed locally.
            if DEPLOYMENT_MODE == "oss":
                logger.info("OSS mode: skipping Dograh remote credit check")
                continue
