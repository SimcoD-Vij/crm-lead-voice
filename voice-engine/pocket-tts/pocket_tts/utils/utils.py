...
def download_if_necessary(file_path: str) -> Path:
    ...
    if file_path.startswith("http://") or file_path.startswith("https://"):
        ...
        if not cached_file.exists():
            headers = {}
            if "huggingface.co" in file_path and token:
                headers["Authorization"] = f"Bearer {token}"
            
            # Use allow_redirects=False to avoid sending token to S3
            response = requests.get(file_path, headers=headers, allow_redirects=False)
            
            if response.status_code in (301, 302, 307, 308):
                redirect_url = response.headers.get("Location")
                # Download from redirect URL without Authorization header
        ...
        try:
            cached_file = hf_hub_download(repo_id=repo_id, filename=filename, revision=revision, token=token)
            return Path(cached_file)
        except Exception as e:
            logger.warning(f"hf_hub_download failed for {file_path_orig}: {e}. Retrying with requests...")
            # Fallback to direct requests.get
            url = f"https://huggingface.co/{repo_id}/resolve/{revision}/{filename}"
            return download_if_necessary(url)
    else:
